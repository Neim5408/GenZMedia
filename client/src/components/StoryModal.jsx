...
const StoryModal = ({ setShowModal, fetchStories }) => {
    ...
    const handleCreateStory = async () => {
        ...
        if (media) {
            const optimizedFile = await compressImage(media);
            formData.append('media', optimizedFile);
        }

        await postApi.post('/story', formData, {
            headers: {
    ...
    return ( 
        <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4">
            <div className="w-full max-w-md">
                ...
                <div className="rounded-lg h-96 flex items-center justify-center realtive" style={{backgroundColor: background}}>
                    {mode === 'text' && (
                        <textarea className="bg-tranparent text-white w-full h-full p-6 text-lg resize-none focus:outline-none" placeholder="what'ts on your mind?" onChange={(e)=>setText(e.target.value)} value={text}/>
                    )}
                    {
