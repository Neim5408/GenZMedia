...
const PostCard = ({ post, authorProfile }) => { 
    ...
    const commentTree = buildTwoLevelCommentTree(comments);
                            ...
                            commentTree.map((comment) => {
                                ...
                                return (
                                    <div key={comment.id} className="space-y-3">
                                        ...
                                        <div className="flex gap-2 text-sm items-start group">
                                            <img 
                                                src={parentUserAvatar} 
                                                alt="avatar" 
                                                className="w-8 h-8 rounded-full object-cover mt-1 border border-gray-50"
                                            />
                                            <div className="flex-1">
                                                <div className="bg-gray-50 hover:bg-gray-100 rounded-2xl px-3 py-2 transition-colors relative">
                                                ...
                                                {/* Action Links */}
                                                <div className="flex gap-4 mt-1 pl-3 text-[11px] text-gray-500 font-semibold">
                                                    <button 
                                                        onClick={() => {
                                            ...
                                            {/* Hapus komentar jika milik sendiri, post milik sendiri, atau admin */}
                                            {(comment.user_id === loggedInUser?.id || isOwnPost || loggedInUser?.role === 'admin' || loggedInUser?.id === 'admin-1' || loggedInUser?.id === 'admin-2') && (
                                                <button 
                                                    onClick={() => handleCommentDelete(comment.id)}
                                                    className="text-gray-400 hover:text-red-500 text-lg self-center opacity-0 group-hover:opacity-100 transition-opacity p-1 font-semibold"
                                        ...
                                        {replyingTo === comment.id && (
                                            <form onSubmit={(e) => handleReplySubmit(e, comment.id)} className="flex items-center gap-2 mt-2 ml-10 animate-fade-in">
                                                <img 
                                                    src={currentUserProfile?.avatar_url || loggedInUser?.avatar_url || 'https://cdn-icons-png.flaticon.com/512/149/149071.png'} 
                                                    alt="user avatar" 
                                                        ...
                                                        <div key={replyL1.id} className="space-y-3">
                                                            ...
                                                            <div className="flex gap-2 text-sm items-start group">
                                                                <img 
                                                                    ...
                                                                    alt="avatar" 
                                                                    className="w-6 h-6 rounded-full object-cover mt-1 border border-gray-50"
                                                                />
                                                                <div className="flex-1">
                                                                    <div className="bg-gray-50 hover:bg-gray-100 rounded-2xl px-3 py-2 transition-colors relative">
                                                                        <div className="flex items-center justify-between">
                                                                        ...
                                                                        <p className="text-gray-700 text-xs mt-1">{replyL1.content}</p>
                                                                    </div>
                                                                ...
                                                                {/* Delete button */}
                                                                {(replyL1.user_id === loggedInUser?.id || isOwnPost || loggedInUser?.role === 'admin' || loggedInUser?.id === 'admin-1' || loggedInUser?.id === 'admin-2') && (
                                                                    <button 
                                                                        onClick={() => handleCommentDelete(replyL1.id)}
                                                                        className="text-gray-400 hover:text-red-500 text-base self-center opacity-0 group-hover:opacity-100 transition-opacity p-1 font-semibold"
                                                                        title="Hapus balasan"
                                                                    >
                                                                        &times;
                                                                    </button>
                                                            ...
                                                            {subReplies.length > 0 && !collapsedComments[replyL1.id] && (
                                                                            ...
                                                                            <div key={replyL2.id} className="space-y-2">
                                                                                ...
                                                                                <div className="flex gap-2 text-sm items-start group">
                                                                                    <img 
                                                                                        ...
                                                                                        alt="avatar" 
                                                                                        className="w-5 h-5 rounded-full object-cover mt-1 border border-gray-50"
                                                                                    />
                                                                                    <div className="flex-1">
                                                                                        <div className="bg-gray-50 hover:bg-gray-100 rounded-2xl px-3 py-2 transition-colors relative">
                                                                                            <div className="flex items-center justify-between">
                                                                                    ...
                                                                                    {/* Delete button */}
                                                                                    {(replyL2.user_id === loggedInUser?.id || isOwnPost || loggedInUser?.role === 'admin' || loggedInUser?.id === 'admin-1' || loggedInUser?.id === 'admin-2') && (
                                                                                        <button 
                                    ...
                                    {commentTree.length === 0 ? (
                                        ...
                                        commentTree.map((comment) => {
                                            ...
                                            return (
                                                <div key={comment.id} className="space-y-3">
                                                    {/* Parent Comment */}
                                                    <div className="flex gap-2 text-xs items-start group">
                                                        <img src={parentUserAvatar} alt="avatar" className="w-7 h-7 rounded-full object-cover border border-gray-50 mt-0.5"/>
                                                        <div className="flex-1">
                                                            <div className="bg-gray-100/50 rounded-2xl px-3 py-2">
                                                                <div className="flex items-center justify-between">
                                                                    <span className="font-semibold text-gray-900 text-[11px]">{comment.full_name || `@${comment.username}`}</span>
                                                                    <span className="text-[9px] text-gray-400">{moment(comment.created_at).fromNow()}</span>
                                                                </div>
                                                                <p className="text-gray-700 mt-1 leading-normal">{comment.content}</p>
                                                            </div>
                                                            <div className="flex gap-4 mt-1 pl-2 text-[10px] text-gray-500 font-semibold">
                                                                <button onClick={() => { setReplyingTo(replyingTo === comment.id ? null : comment.id); setReplyContent(""); }} className="hover:text-indigo-600 transition">Balas</button>
                                                                {replies.length > 0 && (
                                                                    <button 
                                                                        onClick={() => toggleCollapse(comment.id)} 
                                                                        className="hover:text-indigo-600 text-slate-400 transition"
                                                        ...
                                                        {(comment.user_id === loggedInUser?.id || isOwnPost) && (
                                                            <button onClick={() => handleCommentDelete(comment.id)} className="text-gray-400 hover:text-red-500 text-base self-center opacity-0 group-hover:opacity-100 transition p-1 font-semibold">&times;</button>
                                                        )}
                                                    </div>

                                                    {/* Reply input under Parent Comment */}
                                                    {replyingTo === comment.id && (
                                                        <form onSubmit={(e) => handleReplySubmit(e, comment.id)} className="flex items-center gap-2 ml-8 animate-fade-in">
                                                            <input 
                                                                type="text" 
                                                                placeholder={`Balas @${comment.username}...`} 
                                                                value={replyContent}
                                                                onChange={(e) => setReplyContent(e.target.value)}
                                                                className="flex-1 bg-gray-100 rounded-full px-3 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                                                autoFocus
                                                            />
                                                            <button type="submit" disabled={!replyContent.trim() || submittingReply} className="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full text-xs font-bold transition">Kirim</button>
                                                        </form>
                                                    )}

                                                    {/* Nested L1 Replies */}
                                                    {!isCollapsed && replies.map((replyL1) => {
                                                        const replyL1UserAvatar = replyL1.avatar_url || 'https://cdn-icons-png.flaticon.com/512/149/149071.png';
                                                        const subReplies = replyL1.children || [];
                                                        const isL1Collapsed = !!collapsedComments[replyL1.id];

                                                        return (
                                                            <div key={replyL1.id} className="space-y-3 ml-8">
                                                                {/* Level 1 Comment */}
                                                                <div className="flex gap-2 text-xs items-start group">
                                                                    <img src={replyL1UserAvatar} alt="avatar" className="w-6 h-6 rounded-full object-cover border border-gray-50 mt-0.5"/>
                                                                    <div className="flex-1">
                                                                        <div className="bg-gray-100/50 rounded-2xl px-2.5 py-1.5">
                                                                            <div className="flex items-center justify-between">
                                                                                <span className="font-semibold text-gray-900 text-[10px]">{replyL1.full_name || `@${replyL1.username}`}</span>
                                                                                <span className="text-[9px] text-gray-400">{moment(replyL1.created_at).fromNow()}</span>
                                                                            </div>
                                                                            <p className="text-gray-700 mt-0.5 leading-normal">{replyL1.content}</p>
                                                                        </div>
                                                                        <div className="flex gap-4 mt-1 pl-2 text-[10px] text-gray-500 font-semibold">
                                                                            <button onClick={() => { setReplyingTo(replyingTo === replyL1.id ? null : replyL1.id); setReplyContent(""); }} className="hover:text-indigo-600 transition">Balas</button>
                                                                            {subReplies.length > 0 && (
                                                                                <button 
                                                                                    onClick={() => toggleCollapse(replyL1.id)} 
                                                                                    className="hover:text-indigo-600 text-slate-400 transition"
                                                                    ...
                                                                    {(replyL1.user_id === loggedInUser?.id || isOwnPost) && (
                                                                        <button onClick={() => handleCommentDelete(replyL1.id)} className="text-gray-400 hover:text-red-500 text-sm self-center opacity-0 group-hover:opacity-100 transition p-1 font-semibold">&times;</button>
                                                                    )}
                                                                </div>

                                                                {/* Reply input under Level 1 Comment */}
                                                                {replyingTo === replyL1.id && (
                                                                    <form onSubmit={(e) => handleReplySubmit(e, replyL1.id)} className="flex items-center gap-2 ml-8 animate-fade-in">
                                                                        <input 
                                                                            type="text" 
                                                                            placeholder={`Balas @${replyL1.username}...`} 
                                                                            value={replyContent}
                                                                            onChange={(e) => setReplyContent(e.target.value)}
                                                                            className="flex-1 bg-gray-100 rounded-full px-3 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                                                            autoFocus
                                                                ...
                                                                {subReplies.length > 0 && !isL1Collapsed && (
                                                                    <div className="ml-8 pl-3 border-l border-gray-200/80 space-y-3">
                                                                        {subReplies.map((replyL2) => {
                                                                            ...
                                                                            return (
                                                                                <div key={replyL2.id} className="flex gap-2 text-xs items-start group animate-fade-in">
                                                                                    <img src={replyL2UserAvatar} alt="avatar" className="w-5 h-5 rounded-full object-cover border border-gray-50 mt-0.5"/>
                                                                                    <div className="flex-1">
                                                                                       <div className="bg-gray-100/50 rounded-2xl px-2.5 py-1.5">
                                                                                           <div className="flex items-center justify-between">
                                                                                               <span className="font-semibold text-gray-900 text-[10px]">{replyL2.full_name || `@${replyL2.username}`}</span>
                                                                                               <span className="text-[9px] text-gray-400">{moment(replyL2.created_at).fromNow()}</span>
                                                                                           </div>
                                                                                           <p className="text-gray-700 mt-0.5 leading-normal">
                                                                                               {showMention && (
                                                                                                   <span className="text-indigo-600 font-semibold mr-1">@{parentOfReply.username}</span>
                                                                                               )}
                                                                                               {replyL2.content}
                                                                                    ...
                                                                                    {(replyL2.user_id === loggedInUser?.id || isOwnPost) && (
                                                                                        <button onClick={() => handleCommentDelete(replyL2.id)} className="text-gray-400 hover:text-red-500 text-xs self-center opacity-0 group-hover:opacity-100 transition p-1 font-semibold">&times;</button>
                                                                                    )}
                                                                                </div>
                                                                            );
                                                                        })}
