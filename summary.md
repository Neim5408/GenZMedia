...
3. **Database**: A shared PostgreSQL instance (`iae3`) organized using 8 distinct database schemas corresponding to the backend microservices.
...
All services share a single PostgreSQL database (`iae3`) but use qualified schema names to partition tables.
...
The client routes API traffic using a centralized utility script located in [api.js](file:///C:/Users/faisf/Downloads/IAE3/InSight-main/client/src/utils/api.js). Rather than utilizing a gateway, it initiates Axios instances targeting the separate microservice ports directly:
- **`authApi`** -> Port `5000`
- **`userApi`** -> Port `50052`
- **`postApi`** -> Port `50053`
