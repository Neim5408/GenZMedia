...
exports.destroy = async (req, res) => {
  try {
    const { id } = req.params;
    const user_id = (req.body && req.body.user_id) || req.query.user_id; // Safe check for body parameter
...
exports.toggleHide = async (req, res) => {
  try {
    ...
    const comment = await commentService.toggleHide(id, user_id);
    res.status(200).json({ message: "Status penyembunyian komentar diperbarui", comment });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
};
