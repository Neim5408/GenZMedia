...
exports.broadcast = async (req, res) => {
  try {
    ...
    if (!event) return res.status(400).json({ error: "Nama event wajib dikirim" });

    const io = req.app.get('io');
    if (io) {
      io.emit(event, data);
    }
    res.status(200).json({ message: `Event ${event} berhasil di-broadcast`, event, data });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};

exports.markPostCommentsRead = async (req, res) => {
  try {
    const { user_id, post_id } = req.body;
    if (!user_id || !post_id) {
      return res.status(400).json({ error: "User ID dan Post ID wajib dikirim" });
    }
    const result = await notificationService.readPostCommentsNotifications(user_id, post_id);
    res.status(200).json({ message: "Notifikasi komentar postingan telah dibaca", result });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
};