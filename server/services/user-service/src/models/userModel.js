...
const searchUsers = async (searchQuery) => {
  let query = `
    ...
    (SELECT COUNT(*) FROM user_db.follows WHERE following_id = u.id) as followers_count
    FROM user_db.users_profile u
  `;
  let values = [];
  
  if (searchQuery) {
    query += ` WHERE u.username ILIKE $1 OR u.full_name ILIKE $1 OR u.bio ILIKE $1`;
    values.push(`%${searchQuery}%`);
  }
  
  query += ` LIMIT 50`; // Membatasi hasil maksimal 50 orang
