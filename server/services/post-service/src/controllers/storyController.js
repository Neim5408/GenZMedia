...
exports.createStory = (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      // Broadcast real-time new story event
      try {
        ...
        const fullStory = allStories.find(s => s.id === newStory.id);
        ...
        await fetch(`${notifUrl}/notification/broadcast`, {
          ...
          body: JSON.stringify({
            event: 'newStory',
            data: fullStory || newStory
          })
        });
      } catch (broadcastErr) {
        console.warn("Gagal broadcast story baru:", broadcastErr.message);
      }
